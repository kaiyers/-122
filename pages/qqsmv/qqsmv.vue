	...
	<view>
		...
		<view class="uni-list">
			<view class="uni-list-cell" hover-class="uni-list-cell-hover" v-for="(item,index) in list" :key="index">
				<view class="uni-media-list" @tap="readSelf(item.v_id)">
					<image class="uni-media-list-logo" :src="item.mv_pic_url"></image>
					<view class="uni-media-list-body">
						<view class="uni-media-list-text-top">
							<text class="uni-ellipsis">{{item.mv_name}}</text>
						</view>
						<view class="uni-media-list-text-bottom uni-ellipsis" style="margin: 8upx 0;">
							<text style="color: #4CD964;">{{item.singer_name}}</text>
						</view>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			seeklist(){
				uni.request({ 
					url: 'https://api.bzqll.com/music/tencent/search?key=579621905&s='+this.seekText+'&type=mv&limit=20&offset='+this.searchPageNum,
					success: (ret) => {
